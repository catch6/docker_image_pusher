...
# 信号标志：收到 SIGTERM/SIGINT 后置 1
SHUTTING_DOWN=0

mkdir -p "$INPUT_DIR" "$OUTPUT_DIR" "$PROCESSING_DIR" "$(dirname "$LOG_FILE")"

# 优雅停机：清理临时文件后立即退出，不等待转换完成
# processing 目录中的源文件会在下次启动时由 recover_interrupted 恢复
graceful_shutdown() {
    SHUTTING_DOWN=1
    log "收到停止信号，正在退出..."
    rm -f "$PROCESSING_DIR"/*_tmp.mp4
    log "服务已停止"
    exit 0
}
...
convert_video() {
    ...
    # 后台运行 + wait，使 SIGTERM 能中断阻塞，触发 graceful_shutdown
    nice -n 19 ffmpeg -y -stats -stats_period 60 -i "$processing_file" \
        -map 0:v:0 -map 0:a:0? \
        -c:v libx264 \
        -preset slow \
        -crf "$CRF" \
    ...
    if wait $!; then
        ...
        # 信号中断时由 graceful_shutdown 处理，不走失败逻辑
